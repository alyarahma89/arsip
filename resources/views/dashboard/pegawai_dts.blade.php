...
<html lang="id">
<head>
    @include('dashboard.partials.head_pegawai', ['title' => 'Kegiatan DTS'])
</head>
<body>
    @php $fields = ['adm_surat', 'adm_dokumentasi', 'adm_daftar_hadir', 'adm_rundown', 'adm_notulensi', 'adm_laporan', 'adm_materi_instruktur', 'adm_materi_narasumber', 'adm_release', 'adm_sertifikat', 'adm_lapgas']; @endphp
    ...
    <div class="wrapper">
        ...
        <aside class="sidebar">
            ...
            <ul class="nav flex-column">
                ...
                <li class="nav-item">
                    <a href="{{ route('pegawai.dts') }}" class="nav-link active">
            ...
            {{-- 3. FOOTER LOGOUT (BARU - LEBIH RAPI) - MENGIKUTI DASHBOARD --}}
        ...
        <main class="main-content">
            <div class="top-bar">
                <div><h2 class="fw-bold mb-1">Daftar Kegiatan DTS</h2><p class="text-muted mb-0">Kelola administrasi kegiatan Anda.</p></div>
                <div class="d-flex align-items-center gap-3">
                    <form action="{{ route('pegawai.dts') }}" method="GET" class="search-box d-none d-md-flex">
                    ...
                    {{-- NOTIFIKASI BUTTON (SUDAH BERFUNGSI SEPERTI LAPGAS) --}}
                    <div class="dropdown">
            ...
            <div class="table-card mt-4">
                <div class="table-responsive">
                    <table class="table table-hover">
                        <thead><tr><th>No</th><th>Akademi</th><th>Nama Kegiatan</th><th>Tanggal</th><th>Status Adm</th><th class="text-center">Aksi</th></tr></thead>
                        <tbody>
                            @forelse($myEvents as $index => $event)
                            <tr>
                                <td>{{ $myEvents->firstItem() + $index }}</td>
                                <td><span class="badge bg-light text-primary border rounded-pill">{{ $event->akademi }}</span></td>
                                <td>{{ $event->nama_kegiatan }}</td>
                                <td class="text-muted small">{{ \Carbon\Carbon::parse($event->tanggal_mulai)->isoFormat('D MMM Y') }}</td>
                                <td>
                                    @php $done = 0; foreach($fields as $f) { if($event->$f == 'SUDAH') $done++; } $pct = round(($done / 11) * 100); @endphp
                                    <div class="d-flex align-items-center gap-2" style="cursor: pointer" data-bs-toggle="modal" data-bs-target="#modalAdmin{{ $event->id }}">
                                ...
                                <td style="width: 140px;">
                                    <div class="d-flex flex-column gap-2">
                                        @if($event->lokasi && $event->lokasi != '-')
                                            <a href="{{ str_contains($event->lokasi, 'http') ? $event->lokasi : 'https://drive.google.com/drive/search?q='.$event->lokasi }}" target="_blank" class="btn btn-sm btn-primary text-white rounded-pill w-100" style="font-size: 0.75rem;">
                                                <i class="fab fa-google-drive me-1"></i> Folder
                                            </a>
                                        @else
                                            <span class="badge bg-light text-muted border w-100 fw-normal rounded-pill py-1">-</span>
                                        @endif
                                    </div>
                                </td>
                            </tr>
                            @empty <tr><td colspan="6" class="text-center text-muted py-4">Belum ada data.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                <div class="d-flex justify-content-end mt-3">{{ $myEvents->withQueryString()->links('pagination::bootstrap-5') }}</div>
            </div>
        </main>
    </div>

    {{-- Modal Loop Admin Details --}}
    @foreach($myEvents as $event)
    <div class="modal fade" id="modalAdmin{{ $event->id }}" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow" style="border-radius: 20px;">
                <div class="modal-header border-0"><h6 class="modal-title fw-bold">Detail Administrasi</h6><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body p-0">
                    <ul class="list-group list-group-flush">
                        @foreach($fields as $f)
                        <li class="list-group-item d-flex justify-content-between align-items-center px-4">
                            <span class="small text-muted">{{ ucwords(str_replace(['adm_', '_'], ['', ' '], $f)) }}</span>
                            <span class="badge {{ $event->$f == 'SUDAH' ? 'bg-success' : 'bg-warning text-dark' }} fw-normal rounded-pill" style="font-size: 0.7rem;">{{ $event->$f ?? 'BELUM' }}</span>
                        </li>
                        @endforeach
                    </ul>
                </div>
